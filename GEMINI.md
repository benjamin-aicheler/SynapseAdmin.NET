...
- **Status:** Active Development; basic auth, dashboard, room/user management (including creation, memberships, and media), room message history, event reports, registration tokens, federation destinations, server notices, background updates management, advanced media maintenance (cache purging, bulk local deletion, and quarantine protection), multi-language support (EN, DE, FR), and MudBlazor UI are implemented.
